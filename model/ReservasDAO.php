...
class ReservasDAO{
    ...
    public function CategoriasUnidades(){
        $consulta="SELECT id,Nombre from CATEGORIAS WHERE alta=1";
        //echo $consulta;
        return $this->coneccion->SelectDeQuery($consulta);
    }
    ...
    public function TraerSegunDisponibilidad($desde,$hasta,$idCategoria){
        ...
        $query ="INSERT INTO BUSQUEDAS (id,fecha,desde,hasta,idcategoria,ip) values (NULL,'$fecha','$desde','$hasta',$idCategoria,'$ip')";
    ...
    public function ReservasSegunSincronizacion($estado){
        $consulta="SELECT R.id,R.Desde,R.Hasta, R.Fecha_Creada,R.Observaciones,R.Precio,C.Nombre,C.Apellido,C.DNI,C.Mail,C.Celular,C.Domicilio,R.Cant_personas as Personas,L.Nombre as Localidad, P.Nombre as Provincia,R.id_forma_pago as Forma_Pago,U.Nombre as Unidad,U.id as Id_unidad from RESERVAS R INNER JOIN CLIENTES C ON C.id=R.id_cliente INNER JOIN LOCALIDADES L ON L.id=C.id_localidad INNER JOIN PROVINCIAS P ON L.id_provincia=P.id INNER JOIN UNIDADES U ON U.id=R.id_unidad where R.confirmada=1 AND R.Sincronizada=".$estado;
        return $this->coneccion->SelectDeQuery($consulta);
    }
